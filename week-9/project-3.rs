use std::fs::File;
use std::io::{self, Write};

struct Record {
    commissioner: String,
    ministry: String,
    geopolitical_zone: String,
}

fn main() -> io::Result<()> {

    let commissioners = vec![
        "Aigbogun Alamba Daudu".to_string(),
        "Murtala Afeez Bendu".to_string(),
        "Okorocha Calistus Ogbonna".to_string(),
        "Adewale Jimoh Akanbi".to_string(),
        "Osazuwa Faith Etieye".to_string(),
    ];
    
    let ministries = vec![
        "Internal Affairs".to_string(),
        "Justice".to_string(),
        "Defense".to_string(),
        "Power & Steel".to_string(),
        "Petroleum".to_string(),
    ];

    let geopolitical_zones = vec![
        "South West".to_string(),
        "North East".to_string(),
        "South South".to_string(),
        "South West".to_string(),
        "South East".to_string(),
    ];


    let mut records = Vec::new();
    for i in 0..commissioners.len() {
        records.push(Record {
            commissioner: commissioners[i].clone(),
            ministry: ministries[i].clone(),
            geopolitical_zone: geopolitical_zones[i].clone(),
        });
    }


    println!("{:<30} {:<20} {:<15}", "Commissioner", "Ministry", "Geopolitical Zone");
    for record in &records {
        println!(
            "{:<30} {:<20} {:<15}",
            record.commissioner, record.ministry, record.geopolitical_zone
        );
    }

 
    let mut file = File::create("output.txt")?;
    writeln!(file, "{:<30} {:<20} {:<15}", "Commissioner", "Ministry", "Geopolitical Zone")?;
    for record in &records {
        writeln!(
            file,
            "{:<30} {:<20} {:<15}",
            record.commissioner, record.ministry, record.geopolitical_zone
        )?;
    }

    println!("The merged data has been saved to output.txt.");
    Ok(())
}
